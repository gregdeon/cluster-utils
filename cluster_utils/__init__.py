...
def get_header(job_name, allocation, walltime_mins=180, nodes=1, cpus=1, mem_gb=16, gpus=0, gpu_mem_gb=None, array_len=None, output_dir=None, slurm=False):
    """
    ...
    if output_dir is None:
        output_dir = os.path.join(DEFAULT_JOB_DIR, job_name, 'output')
...
def get_default_job_fname(job_name, job_dir=DEFAULT_JOB_DIR):
    """
    By default, write jobs to {DEFAULT_JOB_SCRIPT_DIR}/{job_name}/jobs/{UUID}.pbs

    Arguments:
    - job_name (str): name of job
    - job_dir (str; default=DEFAULT_JOB_DIR): directory for jobs
    """
...
def run_job(fname=None, dry_run=False, verbose=True, **job_kwargs):
    ...
    Arguments:
    - fname (str; default=None): path to job file. If None, then use filename generated by get_default_job_fname.
    ...
    if fname is None:
        ...
        fname = get_default_job_fname(job_name)
